# Generates shaped necklace recipe JSONs for ScalarRPGUtils
# Usage: run from project root:
# ruby src/main/resources/data/scalarrpgutils/recipe/necklaces/generate_necklace_recipes.rb
...
# Pattern: s = string, g = material, # = component
pattern = [
  'ss ',
  'sg ',
  '  #'
...
puts "Generating necklace recipes in: #{out_dir}"
count = 0
materials.each do |mat|
  levels.each do |lvl|
    component_id = "scalarrpgutils:#{mat}_component_level_#{lvl}"
    # 'g' in the pattern stands for gold for all necklaces
    ...
    filename = File.join(out_dir, "#{mat}_necklace_level_#{lvl}.json")
...
puts "Done: Generated #{count} necklace recipes."
